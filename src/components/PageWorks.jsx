    ...
    <main className={`${!timeout && "loadMain"}`}>
      {!timeout ? (
        <div className="pageWorks">
          <div className="title__showType">
            <h1 className="titleHeading">
              Works <span className="worksCount">{projects.length}</span>
            </h1>
            <div className="showType" onClick={() => {
              if(showType === "list"){setShowType("grid")}
              else{setShowType("list")}
            }}>
              <span className={`"showType_1" ${(showType === "list") && "active"}`}>List</span>
              <span className={`"showType_2" ${(showType === "grid") && "active"}`}>Grid</span>
            </div>
          </div>
          <div className="worksContainer">
            {showType === "list" ? (
            <div className="typeList listOfWorks">
